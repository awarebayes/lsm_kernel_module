...
def parse_args():
    ...
    parser.add_argument(
        "-j",
        "--kernel-headers",
        dest="kernel_headers_path",
        help="path to the kernel headers to use for the VM",
        default=os.path.join(os.path.expanduser("~"), "code/linux/usr/include"),
    )
    parser.add_argument(
        "-m",
        "--memory",
    ...
    parser.add_argument(
        ...
        "--ssh-port",
        dest="host_ssh_port",
        help="host port to forward to the SSH port on the VM",
        type=int,
        default=1313,
    )
    parser.add_argument(
        "--vm", dest="vm_path", help="path to the vm image", default="arch_disk.vm"
    )
    parser.add_argument(
...
def main():
    ...
    if not os.path.isdir(args.shared_folder):
        logging.critical(f"shared folder path doesn't exist: {args.shared_folder}")
    qemu_cmd_str = (
        "qemu-system-x86_64 "
